...
double integrandx(double y,double x,double params);
double integrandy(double y,double x,double params);
...
double fx(double x,void *p){
  ...
  gsl_integration_qag(&F, Args.ybound1, Args.ybound2, epsabs, epsrel, allocationsize, 2, giw, &result, &abserror);
...
double velocity_fieldx_c(double * in_array,int size){
  ...
  //  double y=in_array[1];
    ...
    double epsrel=1.49e-8;
    // Perform integration
    gsl_integration_qag(&F, xbounds[0], xbounds[1], epsabs, epsrel, allocationsize, 2, giw, &result, &abserror);
