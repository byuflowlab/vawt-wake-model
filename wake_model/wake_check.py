...
if __name__ == "__main__":
    ...
    for q in range(dec):
        ...
        for d in range(0, 1):
            ...
            wakex,wakey = vawt_wake(xw,yw,dia,rotw[d],ntheta,chord,B,velf,coef0,coef1,coef2,coef3,coef4,coef5,coef6,coef7,coef8,coef9,m,n)
            res = res = Parallel(n_jobs=-1)(delayed(vawt_power2)(i,dia,rotw[d],ntheta,chord,B,velf,af_data,cl_data,cd_data,twist,delta,rho,mu,interp,wakex,wakey) for i in range(np.size(xf)) )
            for i in range(np.size(xf)):
                power_turb[i] = res[i][0]
                if i == 0:
                    velx = res[i][1]
    ...
    plt.savefig('/Users/ning1/Documents/FLOW Lab/Wind Farm Layout Power/wake_check'+str(int(xdn))+'.png')
    # plt.show()
