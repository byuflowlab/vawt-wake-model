...
def veldist(dn,lat,men,sdv,rat,spr,scl1,scl2,scl3):
    f1 = -1./(sdv*sqrt(2.*pi))*exp(-(lat-men)**2/(2.*sdv**2))*(1./(1.+exp(rat*fabs(lat)-spr)))
...
def obj_func(xdict):
    ...
    for i in range(np.size(posdn)):
        if posdn[i] > 0.58:
            vel = veldist(posdn[i],poslt[i],men,sdv,rat,spr,scl1,scl2,scl3)
...
## Optimization
    ...
    param0 = np.array([0.,0.5,30.,20.,0.5,0.1,20.])

    param_l = np.array([None,1e-8,0.,0.,0.,0.,0.])
    param_u = np.array([None,None,None,None,1.,1.,None])
    ...
    if plot == True:
        if paper == True:
            ...
            for i in range(30):
                ...
                exec('xfit = np.linspace(min(pos'+name+'d)-1.,max(pos'+name+'d)+1.,500)')
                exec('plt.plot(velo'+name+'d,pos'+name+'d,color)')
                plt.plot(veldist(xd[i],xfit,men,sdv,rat,spr,scl1,scl2,scl3),xfit,'r-',linewidth=2)
                plt.xlim(0.,1.5)
                # plt.ylim(-4.,4.)
                # plt.legend(loc=1)
...
if __name__ == "__main__":
    ...
    men,sdv,rat,spr,scl1,scl2,scl3 = fit(s,t,length,True,'mac',4,True)
